...
void main()
{
	...
	if (PASSINDEX == 0)	{
		if ((lag == false)||(mod(floor(TIME*60.0+5.0),6.0)<=1.0))	{
			gl_FragColor = IMG_THIS_NORM_PIXEL(buffer10);
		}
		else	{
			gl_FragColor = IMG_THIS_NORM_PIXEL(buffer11);
		}
	}
	else if (PASSINDEX == 1)	{
		if ((lag == false)||(mod(floor(TIME*60.0+3.0),6.0)<=1.0))	{
			gl_FragColor = IMG_THIS_NORM_PIXEL(buffer9);
		}
		else	{
			gl_FragColor = IMG_THIS_NORM_PIXEL(buffer10);
		}
		...
		//	BL - buffer3
			...
			//	TM – buffer1
				...
				color = IMG_NORM_PIXEL(buffer9, tex);
				color.rgb = rgb2hsv(color.rgb);
				color.r = mod(color.r + 9.0 * hueShift, 1.0);
				color.rgb = hsv2rgb(color.rgb);
			}
			else if ((tex.x > 0.5) && (tex.x < 0.75))	{
				tex.x = (tex.x - 0.5) * 4.0;
				...
				color = IMG_NORM_PIXEL(buffer10, tex);
				color.rgb = rgb2hsv(color.rgb);
				color.r = mod(color.r + 10.0 * hueShift, 1.0);
				color.rgb = hsv2rgb(color.rgb);
			...
			//	TL – buffer2
				...
				color = IMG_NORM_PIXEL(buffer11, tex);
				color.rgb = rgb2hsv(color.rgb);
				color.r = mod(color.r + 11.0 * hueShift, 1.0);
				color.rgb = hsv2rgb(color.rgb);
			}			
		}
