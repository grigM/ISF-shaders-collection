/*
{
  ...
  "DESCRIPTION" : "Automatically converted from https:\/\/www.shadertoy.com\/view\/4tjXWD by eddietree.  testing Voronesque",
  "INPUTS" : [
    {
      "NAME" : "iMouse",
      "TYPE" : "point2D"
...
{
    float radius = camZoom;
    float theta = 0.3 + 5.0*mouse.x;// - TIME*0.3;
...
{  
    ...
    float sphere = distSphere(pos, 1.0 + t1*0.75 + sin(((TIME*speed)-animOfset)*2.5+pos.x*1.));// + noise(pos * 1.0 + TIME*0.75);   
    //t1 = max( t1, sphere );
    t1 = sphere;
...
bool renderRayMarch(vec3 rayOrigin, vec3 rayDirection, inout vec3 color ) {
    ...
    {    
        ...
        vec3 normal_distorted = calcNormal( currPos +  noise(currPos*1.5 + vec3(0.0,0.0,sin(((TIME*speed)-animOfset)*0.75))) );
        ...
        float rim = pow(1.0-ndotl, rimBright);
        float rim_distorted = pow(1.0-ndotl_distorted, rimTransp);
