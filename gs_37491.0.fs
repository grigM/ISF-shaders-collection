/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#37491.0"
}
*/
...
void main( void ) {
	...
	for(int i = 0; i < 30; i++) {	   
	   ...
	   if(length(position + ikinci) > 0.35 * sqrt(float(i))) {
	      color = abs(cos(TIME * float(i)));
	   }
	}

	gl_FragColor = vec4( color, 0.2, 0.25, 1.0 );
