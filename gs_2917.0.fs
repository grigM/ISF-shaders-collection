/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#2917.0"
}
*/
...
float f(vec3 o){	
	float a=1.0-fract(cos(o.x)+o.y*2.0)*.005;
	o=vec3(sin(a)*o.x*sin(a)*o.y,sin(a)*o.x-sin(a)*o.y,o.z+(sin(a)-tan(a)*.5));
	return pow(dot(cos(o)*cos(o),vec3(1.5)),2.)-1.5;
}
...
void main(){
	...
	gl_FragColor=vec4(s(vec3(cos((TIME*.1)*speed)*.1,cos(TIME*speed)*1.0,(TIME*speed)), 
	normalize(vec3((2.*gl_FragCoord.xy-vec2(RENDERSIZE.x,RENDERSIZE.y))/vec2(RENDERSIZE.x),1.0))),1);
}