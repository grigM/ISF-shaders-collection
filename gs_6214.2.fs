/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#6214.2"
}
*/
...
vec3 field(vec3 p) {
	p = abs(fract(p)-.5);
	p *= p;
	return sqrt(p+p.yzx*p.zzy)-.01 + mod(p.x,0.05 + 0.02*sin(TIME*5.)) + mod(p.y,0.05 + 0.02*sin(TIME*5.));
}

void main( void ) {
	vec3 dir = normalize(vec3((gl_FragCoord.xy-RENDERSIZE*.5)/RENDERSIZE.x,1.));
	float a = TIME * 0.1;
	vec3 pos = vec3(TIME*0.2,sin(TIME)*0.25,0.0);
