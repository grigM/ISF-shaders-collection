/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#41252.2"
}
*/
...
float map(vec3 o) {
    float f = 1e8;
	float th = sin(TIME/3.14 + o.y*1.0 + 20.0)+sin(TIME*.123);
	//th -= atan(o.x, o.z)*0.25+o.y*.5;
	o.xz *= mat2(cos(th), sin(th), -sin(th), cos(th));
	o.x += sin(TIME*.098+o.y)*.05 + sin(TIME*0.2)*.03;
	f =  box( o, vec3(.5, 4., .5) );
	
	float mu = 10.;
	float add = (sin(o.x*mu)+sin(o.z*mu))*.0002;
	f += add;
    return f;
}
...
void main() {
    ...
    gl_FragColor = vec4(pow(c + vec3(1,-2,3) * 0.01, vec3(0.7)),1);
}